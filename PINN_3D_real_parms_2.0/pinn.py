...
def compute_pinn_loss(model, x_coll: torch.Tensor, y_coll: torch.Tensor, z_coll: torch.Tensor, 
    ...
    # Используем безразмерный коэффициент диффузии = 1
    coef_tensor = torch.tensor(1.0, dtype=x_coll.dtype, device=x_coll.device)
...
def train_pinn(model, diff_coef, num_epochs=200, lr=1e-3, device='cpu'):
    ...
    z = torch.linspace(0, 1, nz, device=device)   # 0-1 глубина (1 радиус пучка)
    t = torch.linspace(0, 1, nt, device=device)   # 0-1 характерное время
