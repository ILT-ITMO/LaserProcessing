...
if __name__ == '__main__':
    ...
    print(f"Пучок: {config.LASER_BEAM_RADIUS*1e6} мкм, сканирование: {config.LASER_SCAN_VELOCITY*1000} мм/с")
    ...
    # Создание сетки для визуализации (в безразмерных координатах)
    nx_plot, ny_plot, nz_plot, nt_plot = 30, 30, 30, 20
    x_plot = np.linspace(-1, 1, nx_plot)  # ±1 радиус пучка
    y_plot = np.linspace(-1, 1, ny_plot)  # ±1 радиус пучка
    z_plot = np.linspace(0, 1, nz_plot)   # 0-1 глубина
    t_plot = np.linspace(0, 1, nt_plot)   # 0-1 характерное время
    ...
    create_animation(U_pred_norm, x_plot, y_plot, z_plot, t_plot, 
                                   ...
                                   'animations/pinn_solution_physical_gaussian.gif')
