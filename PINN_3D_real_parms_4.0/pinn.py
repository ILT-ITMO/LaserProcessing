...
def compute_pinn_loss(model, x_coll: torch.Tensor, y_coll: torch.Tensor, z_coll: torch.Tensor, 
    ...
    # Источник тепла (уже в безразмерной форме)
    ...
    # x_left = -torch.ones_like(x_coll)
    # x_right = torch.ones_like(x_coll)
    ...
    # y_bottom = -torch.ones_like(y_coll)
    # y_top = torch.ones_like(y_coll)
    ...
    # source_at_surface = laser_source_term(x_coll, y_coll, z_top, t_coll)
    # loss_bc += torch.mean((u_top_z_deriv + source_at_surface) ** 2)
    ...
    #                                      create_graph=True, retain_graph=True)[0]
    # loss_bc += torch.mean(u_bottom_z_deriv ** 2)  # ∂u/∂z = 0

    # w_pde, w_ic, w_bc = 1.0, 1.0, 0.1
